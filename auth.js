...
(function () {
    ...
    window.authDoRegister = async function() {
        ...
        if (!/^\d{8,12}$/.test(dni)) {
            authShowError(errEl, 'El DNI/CE debe tener entre 8 y 12 dígitos.'); return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        ...
        try {
            ...
            if (resp.status === 400) {
                authShowError(errEl, (data && data.mensaje) || 'Datos inválidos. Revisa el formulario.'); return;
            }
            if (!resp.ok) {
                authShowError(errEl, (data && data.mensaje) || `Error al crear la cuenta (HTTP ${resp.status}).`); return;
            }

            // Guardar contraseña localmente (el backend no la maneja)
    ...
    window.authDoLogin = async function() {
        ...
        try {
            ...
            // Validar contraseña contra sessionStorage
            const users = JSON.parse(sessionStorage.getItem('pc_users') || '[]');
            const localUser = users.find(u => u.email === email);
            if (localUser && localUser.pass !== pass) {
                authShowError(errEl, 'Contraseña incorrecta.'); return;
            }
            ...
            authShowSuccessAndClose('¡Bienvenido de nuevo, ' + found.nombresCompletos.split(' ')[0] + '!');
            ...
            if (!found) { authShowError(errEl, 'No se pudo conectar al servidor. Verifica tu conexión.'); return; }
